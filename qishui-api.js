...
function getQishuiStatus(cookieText) {
  ...
  return {
    ...
    message: webSession
      ? '汽水音乐扫码登录已连接，可读取歌单与我的喜欢；网页通道支持免费歌曲完整播放，部分歌曲仅提供试听。'
      : tokenConfigured
...
async function fetchQishuiSeoTrack(trackId) {
  return qishuiTrackMetadataCache.wrap('seo|' + trackId, 20 * 1000, async () => {
    const payload = await requestJson(urlWithParams('https://beta-luna.douyin.com/luna/h5/seo_track', {
      track_id: trackId,
      device_platform: 'web',
    }), {
      timeoutMs: 5000,
    ...
    if (!payload.seo_track || !payload.seo_track.track || String(payload.seo_track.track.id) !== String(trackId)) {
      throw new Error('QISHUI_PUBLIC_TRACK_MISSING');
    }
    // The public page also includes comments and recommendations; retain only
    // this track's metadata, playback model and lyrics in the bounded cache.
    return { seo_track: payload.seo_track, track_player: payload.track_player, lyric: payload.lyric };
  });
}
...
async function resolveQishuiPublicPlayback(id, requestedQuality, membership) {
  const payload = await fetchQishuiSeoTrack(id);
  const track = payload.seo_track && payload.seo_track.track;
  if (!track || String(track.id) !== id) throw new Error('QISHUI_PUBLIC_TRACK_MISSING');
  const player = payload.track_player || {};
  const model = qishuiMaybeParseJson(player.video_model);
  const duration = Number(model && model.video_duration) || 0; // seconds, including long tracks
  const fullDuration = Number(track.duration_ms || track.duration) / 1000;
  if (!(duration > 0 && fullDuration > 0)) throw new Error('QISHUI_PUBLIC_DURATION_MISSING');
  const audition = track.audition_info || {};
  const trial = player.video_model_type === 2 ||
    !!(audition.vid && player.media_id === audition.vid && player.media_id !== track.vid) ||
    duration + 2 < fullDuration;
  // This endpoint is requested WITHOUT account credentials. Its returned audio
  // is public full playback or a server-cut preview; do not request another VOD
  // rendition or infer account privileges from these public quality names.
  const { streams } = collectQishuiTrackV2Streams({ track, track_player: player });
  const stream = qishuiBestStreamCandidate(streams.filter(item => /^https:\/\//i.test(item.url)));
  if (!stream) throw new Error('QISHUI_PUBLIC_AUDIO_EMPTY');
  const lyrics = extractQishuiLyrics({ lyric: payload.lyric });
  cacheQishuiLyric(id, lyrics.lyric, lyrics.tlyric, 'qishui-beta-seo-track');
  return {
    provider: 'qishui', playbackMode: 'direct-url',
    url: qishuiUrlWithAuth(stream.url, stream.auth), playable: true,
    source: 'qishui-h5-public', trial,
    trialMessage: trial ? '汽水网页通道仅提供约 ' + Math.round(duration) + ' 秒试听；会员完整播放暂未接通' : '',
    loggedIn: !!membership.sessionValidated, stale: !membership.sessionValidated,
    ...
    level: qishuiNormalizeBitrateKbps(stream.bitrate) > 192 ? 'exhigh' : 'standard',
    quality: stream.quality || stream.format, br: qishuiBitrateForUi(stream.bitrate),
    size: Number(stream.size) || 0, duration, fullDuration,
    clipStart: trial ? Math.max(0, Number(audition.start_time_ms || (track.preview && track.preview.start) || 0) / 1000) : 0,
    requestedQuality, encrypted: !!stream.auth,
  };
}
...
async function handleQishuiSongUrl(opts, cookieText) {
  ...
  try {
    ...
    try { return await resolveQishuiPublicPlayback(id, requestedQuality, checked); }
    catch (_) { /* Keep the original account/source failure when both official paths fail. */ }
  ...
  return qishuiPlaybackCache.wrap(cacheKey, value => value.playable ? 4 * 60 * 1000 : 0, async () => {
    try {
      ...
      if (!entitlementReason) {
        try { return await resolveQishuiPublicPlayback(id, requestedQuality, membership); }
        catch (_) { /* The caller still receives a clear unavailable result below. */ }
      }
