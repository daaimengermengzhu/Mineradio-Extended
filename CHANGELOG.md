...
- 歌单默认按平台返回顺序显示，新增明确的“反向顺序”；详情、3D 歌单架和整单播放共用排序，修复默认反转。
- 兼容酷狗新版歌手、专辑和封面字段；空歌手列表可继续从文件名读取歌手。
- “我喜欢”与无封面歌单显示本地默认封面，加载歌曲后可使用歌曲封面补全。
- 前台默认 60 帧，可选 30 帧或跟随屏幕；隐藏且没有后台视觉需求时停止逐帧绘制，保留桌面歌词独立更新。
- 从上游移植月蚀圣环、雨幕霓虹、折光蝶群、深海绽放，内置视觉预设从 7 个增加到 11 个。
- 汽水音乐新增预览接入：桌面端使用抖音扫码登录、关键词搜索、个人歌单/喜欢列表和歌词。保留本地分享歌单；原生音频使用本机短期票据，缓存限一首并自动释放。
- 修复汽水登录成功但 PC 播放接口返回空响应的问题：自动回退官方网页音频，免费歌曲可完整播放；会员歌曲按平台返回的试听长度播放，明确提示当前通道限制。试听歌词按片段起点对齐，不复用或覆盖整曲节奏缓存。
- 已实测二维码生成/轮询/取消、真实账号登录、公开搜索与原生音频播放；个人歌单内容完整性待核对。会员完整播放、汽水收藏写入和评论发布暂未接通。
- 在禁用其他平台的环境下验证汽水原生音频，包括作者发布时标注独家的 AnliD《The Unfinished Song（慵懒版）》（实测为约 30 秒试听），确认没有同名歌曲换源；另已验证免费歌曲完整播放。正常客户端仍保留带提示的失败换源逻辑。
- 补齐仓库首页、使用教程、发布说明与隐私说明，明确新预设、帧率设置、歌单封面和汽水当前限制。

## v1.1.3 / 2026-08-11

- 第三方音源新增分平台播放优先级，网易云、QQ、酷狗概念版和普通酷狗可分别选择“官方优先”“落雪优先”或“仅官方”。
- “落雪优先”仍会先确认官方账号具备正常播放权限；登录、会员、购买、试听和版权限制不会交给第三方脚本解析。
- 修复从 3D 歌单架直接播放整张歌单时顺序倒置的问题，使整单播放与详情页单曲播放使用同一排序结果。
- 修复关闭主窗口后第三方音源沙箱仍驻留、再次启动可能卡在启动动画的问题，并复用已经启动的本地服务端口。
