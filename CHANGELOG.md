...
- DIY 形态工坊开始接入：形态预设区新增“创建形态”入口，支持用点、线、环、面、螺旋、波、隧道、球面、星尘、卡片等基础粒子编辑 `shapeRecipe` 并保存为本地“我的形态”；新增空白创作画布，点击基础粒子会生成可拖动图层，基础结构包会追加到当前形态而不是覆盖，拖动位置会写回形态配方；自定义形态已接入主 Three.js 场景，可作为真实 3D 点云预设应用到播放器；“我的形态”支持编辑更新、另存、应用、复制、两步确认删除，以及 `.mineradio-shape.json` 导入/导出。
