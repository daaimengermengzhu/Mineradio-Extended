...
Mineradio 扩展版是一款 Windows 桌面沉浸式音乐播放器。原版 Mineradio 已经提供天气电台、搜索播放、歌词舞台、粒子视觉和 3D 歌单架；本扩展版在此基础上补充酷狗概念版、普通酷狗音乐、汽水音乐预览接入（抖音扫码、搜索、歌单与原生播放）、个性化 Home 面板和更自由的 DIY 视觉工坊。
...
- 修复歌单默认倒序，详情、3D 歌单架和整单播放保持相同排序；需要倒序时可手动选择“反向顺序”。
- 修复酷狗部分歌曲的歌手、专辑和封面显示；“我喜欢”有默认爱心封面，加载歌曲后可用歌曲封面补全。
- 从原版移植“月蚀圣环、雨幕霓虹、折光蝶群、深海绽放”四款视觉预设，内置预设共 11 款。
- 新增前台帧率选择，并减少窗口隐藏后不必要的绘制；保留桌面歌词与需要后台运行的视觉功能。
- 汽水音乐支持抖音扫码登录、搜索、歌单读取和原生音频。已在禁用其他平台的环境下实测播放；会员歌曲可能仅提供约 30 秒试听，会员完整播放尚未接通。

详细变更见 [更新记录](./CHANGELOG.md)，操作步骤与限制见 [使用教程](./docs/USAGE_GUIDE.md)。
...
| 汽水音乐（预览） | 支持抖音扫码登录、搜索、歌单读取和分享导入；官方网页通道已实测免费歌曲完整播放，会员歌曲仅提供平台返回的试听片段。 |
| 第三方音源 | 支持用户自行导入符合落雪 API 2.0.0 的可信 `.js` 音源脚本，作为官方播放地址发生技术故障时的可选回退。当前可映射网易云、QQ、酷狗概念版和普通酷狗。 |
| 我的歌单 | 默认保持平台顺序，可选反向顺序、歌手名、本地播放次数；缺失封面时有默认图。 |
| 视觉与占用 | 11 个内置预设；前台可选 30 帧、60 帧或跟随屏幕，隐藏后按后台视觉需求休眠。 |
| 歌词预览 | 软件内歌词默认显示下一句，方便跟唱；可在歌词设置中关闭。 |
| 十段均衡器 | 支持六种预设和自定义曲线，带响度余量与峰值保护；默认关闭，不修改歌曲文件。 |
| Home 个性化 | 左侧“我的音乐海报”支持换本地图片、使用当前封面、页面内编辑文案和重置。 |
| DIY 视觉 | 提供“我的作品”和“形态工坊”，支持用点、线、环、曲线环、螺旋等基础粒子创建本地自定义形态，导入/导出 `.mineradio-visual.json` 和 `.mineradio-shape.json`。 |
...
- 汽水音乐已通过真实账号登录与原生音频播放验证。PC 播放接口失效时自动使用官方网页通道；会员歌曲会明确标注试听，会员完整播放暂未接通。当前不提供汽水收藏写入和评论发布。
- 汽水个人歌单与喜欢列表已接入读取，但内容完整性仍待更多账号验证。原生播放失败时可能尝试其他平台，并显示换源提示；搜索结果标记 `QS` 不代表所有情况下都由汽水播放。
...
- 网易云、QQ、酷狗概念版和普通酷狗可分别设置“官方优先”“落雪优先”或“仅官方”，默认保持“官方优先”。
- 播放器始终先请求歌曲所属平台的官方结果，用于确认当前账号具备正常播放权限；“落雪优先”会在确认可播放后优先尝试已启用脚本，脚本失败则使用官方地址。
- “官方优先”只会在网络、地址失效或格式兼容等技术问题时尝试已启用脚本；“仅官方”完全不调用脚本。
...
如果扩展版的酷狗概念版、普通酷狗、汽水音乐接入或个性化功能帮到了你，也可以通过二创作者支持渠道支持后续维护。
